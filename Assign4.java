...
class DataMatrix implements BarcodeIO
{
   ...
   {
      this.image = image;
      this.actualHeight = image.MAX_HEIGHT;
      this.actualWidth = image.MAX_WIDTH;
      
      /*
      if(!scan(image))
      {
         image = new BarcodeImage();
         actualWidth = image.MAX_WIDTH;
         actualHeight = image.MAX_HEIGHT;
         text = image.toString();
      }
   ...
   public boolean scan(BarcodeImage image)
   {
      ...
      try
      {
         ...
         this.cleanImage();
         actualWidth = computeactualWidth();
         actualHeight = computeactualHeight();
         return true; 
      }
   ...
   private int computeactualWidth()
   {  
      int min_Width = 0;
   ...
   private int computeactualHeight()
   {
      int min_Height = 0;
   ...
   public void displayImageToConsole()
   {
            ...
            row < BarcodeImage.MAX_HEIGHT; row++)
      {
         ...
         {
            temp = boolToChar(image.getPixel(row, col)); //this is where i made a change + 1
